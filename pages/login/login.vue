			...
			submit(){
				...
				db.collection('contacts').where({
					...
					if(res.result.data == ''){
						...
						console.log('已注册')
						getApp().globalData.que2 = res.result.data[0]['que']
						getApp().globalData.ans2 = res.result.data[0]['ans']
						getApp().globalData.loginName = res.result.data[0]['name']
						console.log(getApp().globalData.ans2)
						uni.navigateTo({
